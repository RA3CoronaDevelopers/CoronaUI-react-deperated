...
const stringify = (() => {
  ...
  const shadowSchemaStringify = sjs({
    ...
    depth: attr('number'),
    color: attr('string', n =>
      Array.isArray(n) ? n.map(n => `${n}`).join(' ') : `${n}`
    ),
  });
  ...
  const transformFunctionSchemaStringify = sjs({
    ...
    rotate: attr('string', n =>
      typeof n === 'object' ? martixSchemaStringify(n) : `${n}`
    ),
    rotate3d: attr('string', n => n?.map(n => `${n}`).join(' ')),
    scale: attr('string', n =>
      Array.isArray(n) ? n.map(n => `${n}`).join(' ') : n ? `${n}` : undefined
    ),
    scale3d: attr('array'),
    skew: attr('string', n =>
      Array.isArray(n)
    ...
    translate3d: attr('string', n =>
      Array.isArray(n) ? n.map(n => `${n}`).join(' ') : martixSchemaStringify(n)
    ),
  });
  ...
  const globalSchema = {
    ...
    textAlign: attr('string', n =>
      Array.isArray(n) ? n.map(n => `${n}`).join(' ') : `${n}`
    ),
    textColor: attr('string', n =>
      Array.isArray(n) ? n.map(n => `${n}`).join(' ') : `${n}`
    ),
    backgroundColor: attr('string', n =>
      Array.isArray(n) ? n.map(n => `${n}`).join(' ') : `${n}`
    ),
    zIndex: attr('string', n => `${n}`),
    absolutePosition: generateAroundBoxTemplateSchema(
      attr('string', n => `${n}`)
    ),
    shadow: attr('string', n =>
      typeof n === 'object' ? shadowSchemaStringify(n) : `${n}`
    ),
    width: attr('string', n => `${n}`),
    ...
    transform: attr('string', n =>
      typeof n === 'object' ? transformFunctionSchemaStringify(n) : n
    ),
    order: attr('number'),
